...
(async () => {
	...
	if (input.includes(`icr`) && input.length === 1){
		...
		.then(res => {
			//console.log(res.rows.map(i => ({...i, emailadress: ""})))
			console.table(res.rows.map(i => ({...i, emailadress: ""})));
		});
	...
	//test data node index.js 5 2 2 1 
	//test data node index.js 4 1 1 1
	if (input.includes(`ri`)&& input.length <= 2){
		...
		try {
			...
			const resForQuantities = await client.query('SELECT Quantities FROM InstrumentForRent WHERE instrumentforrent.instrumentrentid = $1 FOR UPDATE;', [input[3]]);
			console.log(resForQuantities)

			if(resForQuantities.rows[0].quantities <= 0){
				throw "Quantity is Zero for instrumentID: " + input[3]; 
			}
			//SELECT COUNT(*) FROM (SELECT * FROM InstrumentCurrentlyRenting WHERE instrumentcurrentlyrenting.personid = 1 FOR UPDATE) as IR;
			const res = await client.query('SELECT COUNT(*) FROM InstrumentCurrentlyRenting WHERE instrumentcurrentlyrenting.personid = $1 AND RentTerminated = false;', [input[1]]);
			console.log(res);
			...
			await client.query('INSERT INTO InstrumentCurrentlyRenting (PersonID, StudentID, InstrumentRentID, InstrumentID, DateRented, RentTerminated) VALUES ($1 ,$2 ,$3, $4, NOW(), false);', input.slice(1));
	...
	if (input.includes(`ticr`) && input.length <= 2){
		...
		try {
			...
			//console.log(r1);
			const r2 = await client.query('UPDATE InstrumentCurrentlyRenting AS i SET RentTerminated = true WHERE i.instrumentrentid = $2 AND i.personid = $1;', input.slice(1));
			//console.log(r2),
			await client.query('COMMIT')
		  } catch (e) {
			await client.query('ROLLBACK')
